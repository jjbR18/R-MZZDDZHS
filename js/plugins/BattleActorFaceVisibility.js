...
//改変　KICHUREA
    Window_Face.prototype.drawActorFace = function(actor) {
        var meta = $dataClasses[actor._classId].meta;
///       if(meta.stand_picture_x){ 
///        this.x = 50 - (meta.stand_picture_x * 1);
///        } else {
///        this.x = 50;
///       } 
       
       
        if (meta != null && meta.stand_picture) {
 ///ロードエネミーに改変
 if ($gameSwitches.value(18) == true) {this.drawPicture(getArgString('N' + meta.stand_picture), ImageManager.loadEnemy.bind(ImageManager)); } 
 if ($gameSwitches.value(18) == false){this.drawPicture(getArgString(meta.stand_picture), ImageManager.loadEnemy.bind(ImageManager)); }    
